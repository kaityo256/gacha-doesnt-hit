...
この作業を続けて、44個すべての数字が消えたとき、つまり44種類すべてのポスターが揃ったときに、サイコロを何回振ったかを記録します。これが、「コンプリートまでにCDを何枚買ったか」に対応します。

シミュレーションコードはとても短く書けます。以下はCDを買い続けて44種類のポスターをコンプリートするまでに何枚必要だったかを調べる試行を1万回行い、その平均を表示するコードです。
...
実行してみると、毎回結果は少しずつ変わりますが、およそ190〜193枚くらいの値になります。
...
ここまで来ると、多くの人は、「あと少しで全部そろう」と思うでしょう。しかし、先ほど計算したように、44種類すべてのポスターをそろえるためには、平均して約192.4枚のCDが必要になります。つまり、100枚買ってコンプリート率が90%くらいになったとしても、そこで終わりが近いわけではありません。むしろ、コンプ率90%で、ちょうど道半ばくらいなのです。
...
何かに使ってしまったお金や時間は、あとから取り戻すことができません。目的を達成できたのなら、かけたお金や時間は「報われる」ことになりますが、目的を達成できなくても、お金も時間も帰ってきません。このように、もう戻ってこない費用がサンクコストです。
...
このような場合にも、シミュレーションはとても役に立ちます。厳密な数式を導出できなくても、とりあえずシミュレーションしてみることで、目的のカードを手に入れるのにおよそ何回ガチャを回さないといけないのか、運が悪いとどれくらい手に入らないのか、といったことを簡単に調べることができます。
...
ここで紹介した「アイドルの44枚のCDイベント問題」のように、「ランダムに手に入るアイテムを、指定の種類揃えたら特別な景品が手に入る」という仕組み、いわゆる「コンプガチャ」は、景品表示法という法律で規制されています。具体的には、昭和52年の公正取引委員会告示第3号に「懸賞による景品類の提供に関する事項の制限」があり、その第5項に、「前三項の規定にかかわらず、二以上の種類の文字、絵、符号等を表示した符票のうち、異なる種類の符票の特定の組合せを提示させる方法を用いた懸賞による景品類の提供は、してはならない。」と定められています。この法律によれば、例えばガチャで「頭、右腕、左腕、右足、左足」を全て引けたら強力なキャラが手に入る、といった仕組みも違法になる可能性があります。

ここで紹介した「アイドルの44枚のCDイベント問題」は、実際に起きた出来事をもとにした例で、大学の講義でも題材として取り上げています。講義では平均必要枚数を求めるプログラムを課題として提出させると共に、「コンプガチャは法律で規制するべきかどうか」についての意見もレポートで提出させています。すると、「コンプガチャは規制すべき派」と「条件を明示しているなら規制すべきでない派」がきれいに分かれます。

何かを法律で規制するべきかどうかを決めるのはとても難しいことです。そのためには、我々の社会がどのようなものであってほしいか、を考える必要があります。合法派の意見でよく見られたのは「自己責任」という言葉でした。それぞれのレアアイテムの排出確率に嘘がなければ、それらをコンプするのに必要なガチャの回数はちゃんと計算できることになります。その上でガチャを回すなら、それは自己責任であろう、という意見です。一方、規制派の意見は、「たとえ表示に全く嘘がなかったとしても、それに騙される人が出てくる社会は不幸である。だから規制すべき」という意見です。そのどちらの意見にも合理性があります。
