...
この微分方程式に従う車を円形のサーキットにたくさん配置してから動かして、しばらく放っておきましょう。以下の図は、パラメータを変えてシミュレーションした結果です。


![最適速度模型の振る舞い](fig/ovmodel.png)

図1: 最適速度模型のシミュレーション。最初にサーキットに車を等間隔に配置し、一斉に運転を始める。車間距離が十分に空いていたり、ドライバーの反射神経が良いと渋滞は発生しない(自由流相)が、車間距離が狭くなると渋滞が発生する(渋滞相)。

車の台数が少ない時には何も起きません。ただ等間隔に車が並んでいるだけです。どこかの車がブレーキを踏んでも、車はお互いの車間距離をなるべく空けようとするので、しばらくすると等間隔に戻ります。車が自由に流れているので、この状態を**自由流相**と呼びます。

一方、車の数を増やしていくと、だんだんと「どこかの車がブレーキを踏んだ時の影響」が大きくなります。そして、ある数を超えた時、「どこかの車が少しでもブレーキを踏むと、後続の車が次々とブレーキを踏み、ついにはほとんど止まってしまう」という状態になります。そして、ほとんど止まってしまう渋滞領域と、自由に運転する領域に分かれてしまいます。こちらは**渋滞相**と呼ばれます。
...
このモデルでは、パラメータ$a$と車の数(正確には平均車間距離)によって、渋滞が起きるかどうかが決まります。こんな簡単なモデルで、渋滞の発生という、実際に社会で起きる現象をシミュレーションすることができました。
