...
ルールは完全に分かっているのに結果を想像するのが難しい例として、シャーレの中のバクテリアの数のシミュレーションをしてみましょう。シャーレの培地にバクテリアを発生させます。このバクテリアは、一定期間ごとに子孫を残して死にます。シャーレの培地の栄養価が高いほど子孫をたくさん残すことができます。また、シャーレの広さには限りがありますから、このシャーレの世界に住めるバクテリアの個体数には上限があると考えられます。このバクテリアの数の時間発展を数式で表してみましょう。

$i$世代目のバクテリアの個体数を$N_i$とします。このシャーレに住める個体数の最大値を$N_\mathrm{max}$としましょう。もしも、現在の個体数$N_i$が、最大の個体数$N_\mathrm{max}$に比べて十分少なければ、このバクテリアは周りを気にせず一匹あたり$a$匹の子どもを残します。ただし$a$は培地の栄養価に依存する比例係数で、栄養価が高いほど大きな値を持ちます。一方、もし現在の個体数$N_i$が、個体数の上限値$N_\mathrm{max}$に近ければ、ほとんど子どもを残すことができません。以上から、次のように世代ごとの個体数に関する漸化式を考えることができます。

$$
N_{i+1} = a\left(1 - \frac{N_i}{N_\mathrm{max}} \right) N_i
$$

もし、現在の個体数$N_i$が個体数の上限値$N_\mathrm{max}$に比べて十分に小さければ、次の世代の個体数は$N_{i+1} = a N_i$と、ほぼ$a$倍になります。一方、もし現在の個体数$N_i$が、個体数の上限値$N_\mathrm{max}$に近ければ、$N_{i+1}$はほぼゼロとなります。先ほど考えた条件を満たしていることがわかります。

この式を少し整理してみましょう。シャーレの栄養価の高さを表すパラメータ$a$が大きいほど子どもをたくさん残せるのですから、$a$が大きいほど個体数が多く、$a$が小さいほど個体数が小さくなることが想像されます。それを考えるために、個体数をその上限で割った値$n_i = N_i / N_\mathrm{max}$を考えます。$n_i$は、$i$世代目の個体数が個体数上限に比べてどのくらいの割合になっているかを表す数です。個体数上限に達したら1、十分少なければ0となります。このように大きさを考えやすい値にそろえる操作を**規格化**と呼び、物理学においては重要な考え方です。先ほどの関係式の両辺を個体数の上限値$N_\mathrm{max}$で割って整理すると、こんな式になります。

$$
n_{i+1} = a (1 - n_i ) n_i
$$

だいぶ簡単になりました。さて、この小さなシャーレの世界において、バクテリアがずっと世代を重ねていったらどうなるかを考えてみましょう。世代を重ねて、個体数が変わらなくなる状態になったとします。これを**定常状態**と呼びます。もしシャーレの世界が定常状態になったら、個体数が世代によらなくなるのでうから、$n_i = n_{i+1} = n$が成り立ちます。これを先程の式に代入すると、
...
となります。つまり、栄養価パラメータ$a$が高いほど$n$は1に近づきます。$n$は個体数上限に対する割合ですから、$n$が1に近づくということは、個体数の上限に近づくということです。一方、$a$が小さくなると、$n$は0に近づいて行き、$a=1$の時にゼロ、すなわち全滅します。実際にそうなるかシミュレーションで確認してみましょう。そのコードを図に示します。
...
このコードは、$n$の初期値0.1、つまり上限個体数の10%の状態から、様々な栄養価パラメータ$a$の値で1000世代計算させ、最後の100世代をプロットするものです。実行結果は以下の図のようになります。
...
それぞれの栄養価パラメータ$a$の値でそれぞれ最後の100世代分の$n_i$の値を描画していますが、すべて重なっています。これは定常状態になっていることを示しています。また、落ち着く値は理論値である$n=1-1/a$に一致しています。特に不思議なことは置きていません。

では、栄養価パラメータ$a$について、もう少し大きな値で試してみましょう。$3<a<4$の時の個体数のプロットは以下のようになります。
...
急に不思議な図が現れました。この図の見方ですが、例えば$a=3.2$のところで、個体数割合$n_i$が$0.5$と$0.8$あたりを振動するようになったため、最後の100世代をプロットすると、その2つの値が表示されています。
