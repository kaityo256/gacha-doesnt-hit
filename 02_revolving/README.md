...
この式を少し整理してみましょう。個体数 $N_i$ そのものを扱う代わりに、個体数の上限 $N_\mathrm{max}$ に対する割合
...
ただし、ここで注意したいのは、この計算で求めた値はあくまで「定常状態の候補」だということです。実際に世代を重ねたときにその値に落ち着くかどうかは、パラメータ $a$ の値によって変わります。そこで、シミュレーションで確かめてみましょう。次のコードでは、初期値を $n=0.1$ として、さまざまな $a$ の値について1000世代分の計算を行います。そして、最後の100世代の値をプロットします。
...
このコードでは、$a$ の値を少しずつ変えながら計算しています。それぞれの $a$ について、最初の900世代は捨てて、最後の100世代だけを図に描いています。これは、十分に時間が経った後に個体数がどのような振る舞いをするかを見るためです。
...
と一致します。つまり、この範囲では、シミュレーションの結果は定常状態の計算とよく合っています。ここまでは、直感どおりの結果だと言えるでしょう。

では、栄養価パラメータ $a$ をもう少し大きくしてみましょう。$3<a<4$ の範囲で同じように計算すると、次のような図になります。
...
たとえば $a=3.2$ のあたりを見ると、点が1つの値には集まっていません。代わりに、$n$ が約0.5付近と約0.8付近の2つの値を行き来しています。これは、個体数が一定値に落ち着くのではなく、世代ごとに2つの値の間を振動していることを表しています。
...
というとても単純な式だったということです。それにもかかわらず、パラメータ $a$ を変えるだけで、定常状態、周期的な振動、そして複雑な変化が現れます。
...
それにもかかわらず、パラメータ$a$の値によっては、個体数の変化が非常に複雑になり、長い時間の後にどのような値になるかを予測することが難しくなります。特に、最初の値がほんの少し違うだけで、しばらく後にはまったく異なる結果になることがあります。科学でいうカオスとは、このように「決まったルールに従っているにもかかわらず、未来の予測がとても難しくなる現象」のことです。ロジスティック写像は、このカオスをとても単純な式から見ることができる代表的な例です。
